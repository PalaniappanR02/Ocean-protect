...
export function AnalystMap(){
  ...
  const {data:regions}=useQuery({queryKey:['regions'],queryFn:()=>regionService.list()});
    return <div className="animate-fade-in"><PageHeader title="Analyst Hazard Map" description="Review citizen reports, incidents, and coastal priority regions" icon={Map}/><Card><CardContent className="p-0"><MapShell reports={reports?.items||[]} incidents={incidents||[]} regions={regions||[]} className="h-[700px] rounded-lg"/></CardContent></Card></div>;
}
