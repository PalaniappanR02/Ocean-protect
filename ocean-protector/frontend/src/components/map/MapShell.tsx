  ...
  return (
    <div className={`relative h-full ${className || ''}`}>
      <Suspense fallback={<MapSkeleton />}>
        <HazardMap reports={reports} incidents={incidents} regions={regions} onMarkerClick={(id:string)=>{ const s = incidents.find((i:any)=>i.id===id) || reports.find((r:any)=>r.id===id); setSelected(s || null); }} className="h-full rounded-lg" />
      </Suspense>

      {/* Floating overlays */}
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-4 top-4 pointer-events-auto"><MapToolbar onReset={()=>{}} onLocate={()=>{}}/></div>
        <div className="absolute left-4 top-28 pointer-events-auto"><WeatherWidget/></div>
        <div className="absolute right-4 top-4 pointer-events-auto"><FloatingLegend items={[{color:'var(--color-success)', label:'Low'},{color:'var(--color-warning)', label:'Warning'},{color:'var(--color-danger)', label:'Critical'}]} /></div>
        ...
        {filtersOpen && <div className="absolute left-0 top-0 bottom-0 pointer-events-auto"><MapFilters onClose={()=>setFiltersOpen(false)} /></div>}
      </div>
    </div>
  );
};
